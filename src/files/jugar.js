...
var noteITOT = document.querySelector('.noteITOT')

if(localStorage.getItem('ITOTWorld')){
  selectWorld = localStorage.getItem('ITOTWorld')
  localStorage.setItem('ITOTWorld', false)
}
...
background[0].src = "https://media.discordapp.net/attachments/961066933669728257/992492116662046851/unknown.png?width=757&height=469"
...
background[2].src = "https://media.discordapp.net/attachments/961066933669728257/992492227798515884/unknown.png?width=775&height=469"
background[3].src = "https://media.discordapp.net/attachments/961066933669728257/992491941637914714/unknown.png?width=760&height=469"
...
background[10].src = "https://media.discordapp.net/attachments/961066933669728257/989658014975926333/unknown.png?width=799&height=492"
var objects = [
  new Image(),
  new Image(),
  new Image(),
...
objects[6].src = "https://media.discordapp.net/attachments/961066933669728257/989659947870617610/unknown.png?width=337&height=492"
...
function world() {
  ...
  if (selectWorld == 1) {
    ...
    noteITOT.style.visibility = "hidden"
  }else if(selectWorld == 200) {
    ctx.drawImage(background[10], 0, 0, canvas.width, canvas.height)
    dialog.style.visibility = "visible"
    localStorage.setItem('ITOTWorld', 200)
    if(!localStorage.getItem('ITOTMision'))return
    if(localStorage.getItem('ITOTMision') == "2")return dialog.innerHTML = `¡Muy bien JA, JA! ¿Qué tal si empezamos la segunda misión? Espera, yo decido por ti. JEJE.<br><button onclick="location.href = '/memoryTime'">Siguiente Misión</button>`
    if(localStorage.getItem('ITOTMision') == "3")return dialog.innerHTML = `Felicidades pato del Tiempo 253, disfruta esas monedas JA JA! Sigamos con la siguiente misión, pero ten cuidado… te recomiendo mirar la historia.<br><button onclick="location.href = '/cardsTempy'">Siguiente Misión</button><button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#historyModal">Ver historia</button>`
    if(localStorage.getItem('ITOTMision') == "4")return dialog.innerHTML = `¡FELICIDADES PATO DEL TIEMPO 253! Estoy orgulloso de tí, haz logrado vencer al Guardian del Tiempo... Aunque ya no es muy guardian JA JA. Como ahora te tengo confianza, me presento: Soy Jeek, un pato de esta misma isla, pero del tiempo 286 en el futuro. Fué un gusto conocerte. ¡Adios!<br><button onclick="localStorage.removeItem('ITOTWorld');localStorage.removeItem('ITOTMision');location.href = '/play';">Terminar</button>`
  }
}
...
function addObject(){
  ...
  if(selectWorld == 200){
    ctx.drawImage(objects[6], 60, canvas.height-220, 220, 300)
  }
}
...
function addItems(){
  var itemsJ = JSON.parse(localStorage.getItem('outfitSave'))
    ...
    for(var i = 0; i < itemsJ.length; i++){
      divItems.innerHTML += `
     <div class="item" style="padding:5px;border-radius:5px;" onclick="ponerItem('${itemsJ[i].id}', '${itemsJ[i].tipo}')"><img src="${itemsJ[i].img}" width="100%"></div>`
    }
}
